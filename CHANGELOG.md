...
- 四类术语匹配统一排除 LaTeX 命令名、引用/标签/引文参数、行内与行间数学，以及 equation、align、gather 环境，同时保持既有 schema、kind 与退出码语义。
- 合成回归覆盖零差分、已有形式、命令名、键、数学区域、句首/句中大小写和 forbidden synonym；这些结果只验证机械检查路径，不构成真实稿件能力或论文质量证据。
