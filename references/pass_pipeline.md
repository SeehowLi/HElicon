...
For P3 only, `glossary_terms` is the authorized output domain and is exempted from immutable-set blocking; any nonzero `numbers_with_units`, `latex_keys`, `math`, `figure_table`, or `claim_scope` count still rolls back P3. P3 also proceeds only when both terminology checks reported by `check_pass_scope.py` pass: the forward `check_terminology_freeze(BEFORE, AFTER)` check must introduce zero forbidden forms, and the full residual scan of AFTER must leave zero forbidden forms. A failure of either check rolls back P3 and identifies `terminology_forward_check` or `terminology_residual_check` in the trailer. For P4-P7, no category is exempted and `glossary_terms` remains blocking exactly as before.

Every trailer must include `p3_glossary_exemption=applied|not-applicable`. Its immutable-set entry must expose both the effective pass-scope exit and the exempted count, for example `check_immutable_set=exits[P3:0(glossary_terms=3 exempted),...]`; the machine JSON retains `immutable_raw_exit_code`, and an exemption may never hide the raw count.
