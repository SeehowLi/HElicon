...
1. Run the normative pass-aware implementation `python -B scripts/check_pass_scope.py --pass <P3|P4|P5|P6|P7> <BEFORE> <AFTER> --glossary <MERGED>`. Exit 0 proceeds; exit 1 rolls back under Iron Rule 6; exit 2 is an input or configuration error and must stop with a report rather than count as passing.

For P3 only, `glossary_terms` is the authorized output domain and is exempted from immutable-set blocking; any nonzero `numbers_with_units`, `latex_keys`, `math`, `figure_table`, or `claim_scope` count still rolls back P3. The forward `check_terminology_freeze(BEFORE, AFTER)` check must introduce zero forbidden forms. Residual scans are warning evidence measured with the same absolute ruler on BEFORE and AFTER: only `terminology_residual_delta > 0` rolls back P3 with reason `residual_increased`; a zero or negative delta is nonblocking and is reported as unchanged or improved. Absolute residual counts include pre-existing false positives from legitimate capitalization and legitimate noun phrases, so they are not absolute thresholds and may be used only through the same-ruler differential. For P4-P7, no category is exempted and `glossary_terms` remains blocking exactly as before.

Every trailer must include `p3_glossary_exemption=applied|not-applicable` and report `terminology_residual_before`, `terminology_residual_after`, and `terminology_residual_delta`. Its immutable-set entry must expose both the effective pass-scope exit and the exempted count, for example `check_immutable_set=exits[P3:0(glossary_terms=3 exempted),...]`; the machine JSON retains `immutable_raw_exit_code`, and an exemption may never hide the raw count.
